(()=>{
  ...
  const api={
    ...
    createClub:async p=>{const x={id:uid(),name:p.name,city:p.city||""};state.clubs.push(x);persist();return clone(x)},
    ...
    createTeam:async(did,p)=>{const id=(p.group||"X")+Math.ceil(Math.random()*90);const t={id,divisionId:did,name:p.name,club:p.club||"Tự do",group:p.group||"A",seed:p.seed||null,w:0,l:0,pf:0,pa:0};state.teams.push(t);state.teamPlayers[id]=[];persist();return clone(t)},
