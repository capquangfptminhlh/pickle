...
function render(){
  ...
  $("#publicTours").innerHTML=state.tournaments.map(t=>`<article class="tour-card"><div class="tour-cover"><strong>${t.format||"Tournament"}</strong>${badge(t.status)}</div><div class="tour-body"><h3>${t.name}</h3><div class="meta"><span>📅 ${t.date}</span><span>📍 ${t.venue}</span><span>👥 ${t.teams} đội</span></div></div></article>`).join("")||'<div class="empty">Chưa có giải công khai.</div>';
