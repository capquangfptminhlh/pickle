...
function renderCore(){
  ...
  $("#publicTours").innerHTML=state.tournaments.map(t=>
    '<a class="tour-card" href="/tournament.html?id='+encodeURIComponent(t.id)+'" style="text-decoration:none;color:inherit">'+
      '<div class="tour-media"><img src="https://images.pexels.com/photos/17299530/pexels-photo-17299530.jpeg?auto=compress&cs=tinysrgb&w=1400" alt="'+esc(t.name)+'"><div class="tour-media-shade"></div><div class="tour-cover"><div><small>TOURNAMENT</small><strong>'+esc(t.format||"Tournament")+'</strong></div>'+badge(t.status)+'</div></div>'+
...
function renderPosts(){
  ...
  el.innerHTML=posts.slice(0,6).map(p=>
    ...
    (p.cover_url?'<img src="'+esc(p.cover_url)+'" alt="'+esc(p.title)+'">':'<img src="https://images.pexels.com/photos/29439343/pexels-photo-29439343.jpeg?auto=compress&cs=tinysrgb&w=1400" alt="'+esc(p.title)+'">')+
