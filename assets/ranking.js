...
function render(){
 ...
 $("#rankingRows").innerHTML=rows.map((p,i)=>`<div class="standing-row" style="grid-template-columns:60px 1.8fr 1fr 1fr"><b>${i+1}</b><span><b>${p.full_name}</b>${p.nickname?`<small style="display:block;color:#708078">${p.nickname}</small>`:""}</span><span>${p.club_name||"Tự do"}</span><b>${Number(p.rating||0).toFixed(3)}</b></div>`).join("")||'<div class="empty">Không có VĐV phù hợp.</div>';
}
async function load(){
